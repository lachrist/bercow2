...
const performAction = async (action, plugin) => {
  if (action.stage === "digest") {
    return {
      stage: "digest",
      inner: await plugin.digest(action.specifier),
    };
  } else if (action.stage === "link") {
    return {
      stage: "link",
      inner: await plugin.link(action.specifier),
    };
  } else if (action.stage === "validate") {
    await log(`Validating ${action.specifier}...\n`);
    const inner = await plugin.validate(action.specifier);
    await log(
      fromEither(

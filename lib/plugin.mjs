...
import { catchMaybeAsync, fromMaybe } from "./util/maybe.mjs";
...
export const cookPlugin = async (
  ...
  mapRight(instantiateCache(serial), (cache) => ({
        ...
        async (hash) =>
          fromMaybe(
            await await cacheMutMap(cache.lint, hash, () =>
              catchMaybeAsync(lint(specifier))
            ),
            () => makeRight(hash),
    ...
    test: async (specifier, hash) =>
      await await cacheMutMap(cache.test, hash, () =>
        catchMaybeAsync(test(specifier))
      ),
